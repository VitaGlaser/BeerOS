...
namespace AsnPlus::Cloud
{
    ...
    void toJson( TimeConfig & config, cJSON * json )
    {
    ...
    void fromJson( TimeConfig & config, cJSON * json )
    {
        ...
        if ( tz && cJSON_IsString( tz ) ) strncpy( config.timezone, tz->valuestring, TimeConfig::TIMEZONE_LENGTH - 1 );

        cJSON * tzName = cJSON_GetObjectItem( json, TimeConfigJson::TIMEZONE_NAME_TAG );
        if ( tzName && cJSON_IsString( tzName ) )
            strncpy( config.timezoneName, tzName->valuestring, TimeConfig::TIMEZONE_NAME_LENGTH - 1 );
    }
